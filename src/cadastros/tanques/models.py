...
class Combustivel(Base):
    ...
    def __str__(self):
        return f'{self.nome_combustivel} - {self.empresa}'



class Tanque(Base):
    tipo_combustivel = models.ForeignKey(Combustivel, verbose_name='Tipo de Combustivel', on_delete=models.SET_NULL, null=True, blank=True)
    identificador_tanque = models.CharField(verbose_name='Identificador', max_length=24, unique=True)
